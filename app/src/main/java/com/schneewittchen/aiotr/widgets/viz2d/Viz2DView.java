package com.schneewittchen.aiotr.widgets.viz2d;
...
import com.schneewittchen.aiotr.ui.views.widgets.PublisherLayerView;
...
public class Viz2DView extends WidgetGroupView {
    ...
    public void onNewData(RosData data) {
        if (data.getTopic().name.equals("/tf")) {
            layerView.onNewData(data);
        } else if (data.getTopic().name.equals("/map")) {
            layerView.onNewData(data);
            Log.d(TAG, "new data from /map topic");
        } else {
            layerView.onNewData(data);
        }
    }
